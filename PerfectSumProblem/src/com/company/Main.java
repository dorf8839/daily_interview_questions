...
public class Main {
    ...
    //A recursive function to print all subsets with the help of
    //dp[][].  Vector p[] stores current subset.
    static void printSubsetsRec(int[] arr, int i, int sum, ArrayList<Integer> p){
        //If we reached end and sum is non-zero. We print p[] only if arr[0]
        //is equal to sum OR dp[0][sum] is true
        ...
        if(dp[i-1][sum]){
            //Create new vector to store path
